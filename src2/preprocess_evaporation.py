...
class evaporation_transformer(BaseEstimator, TransformerMixin):
    '''
    ...
    def __init__(self, geo :str = 'Location', col_select : str = 'Evaporation', col_target : str = 'RainTomorrow', method : str = 'median'):
        self.dict_evap = {}
        self.geo = geo
    ...
    def fit(self, X, y = None):
        ...
        X_drop.columns = ['Location_df', 'Location_df_evap']
        X_drop['ratio'] = (X_drop['Location_df_evap'] / X_drop['Location_df'])
        X_drop = X_drop[X_drop['ratio'] > 0.7]

        for i in X_drop.index:
            for j in X[self.col_target].unique():
                self.dict_evap[i, j] = X[(X[self.geo] == i) & (X[self.col_target] == j)][self.col_select].agg(self.method)
