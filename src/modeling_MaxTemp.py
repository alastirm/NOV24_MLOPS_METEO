...
# Visualisation de la série temporelle de MaxTemp
plt.figure(figsize=(15, 7))
plt.plot(df_location["Date"], df_location["MaxTemp"], linewidth=0.8, label="Température maximale", color="firebrick") 
plt.xticks(rotation=45)
plt.xlabel("Date")
plt.ylabel(f"Température maximale (°C)")
plt.title(f"MaxTemp {station_name} - SARIMA : Évolution de la variable", fontsize=15)
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"1. {station_name}_MaxTemp_Évolution.png"))
plt.close() 

# Décomposition de la série temporelle de MaxTemp
plt.figure(figsize=(15, 7))
plt.rcParams.update({"axes.prop_cycle": plt.cycler("color", ["red", "red", "red", "red"])})
result = seasonal_decompose(df_location["MaxTemp"], model="additive", period=365)
fig = result.plot()
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"2. {station_name}_MaxTemp_TimeSeries_Décomposition.png"))
plt.show()

# Diviser les données en ensemble d'entraînement et de test
train = df_location["MaxTemp"][:int(0.8 * len(df_location))]
test = df_location["MaxTemp"][int(0.8 * len(df_location)):]

# Créer un DataFrame des features
train_features = df_location.drop(columns=["Date", "Location", "MaxTemp"])[:int(0.8 * len(df_location))]
test_features = df_location.drop(columns=["Date", "Location", "MaxTemp"])[int(0.8 * len(df_location)):]

# Afficher ACF et PACF de MaxTemp
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"3. {station_name}_MaxTemp_ACF_PACF.png"))
plt.close() 
...
results_file = os.path.join(output_dir, f"4. {station_name}_MaxTemp_SARIMA_Results.txt")
...
# Visualiser les prédictions du modèle SARIMA pour MaxTemp
plt.figure(figsize=(15, 7))
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"5. {station_name}_MaxTemp_SARIMA_Prédictions.png"))
plt.close() 

# Évaluation du modèle SARIMA pour MaxTemp
...
evaluation_file = os.path.join(output_dir, f"6. {station_name}_MaxTemp_Évaluation_modèle.txt")
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"7. {station_name}_MaxTemp_Prophet_Components.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"8. {station_name}_MaxTemp_Prophet_Predictions.png"))
plt.close()
...
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"9. {station_name}_MaxTemp_Prophet_Predictions_vs_Real.png"))
plt.close()
...
# Sauvegarder l'évaluation dans un fichier texte
evaluation_file_prophet = os.path.join(output_dir, f"10. {station_name}_MaxTemp_Prophet_Évaluation_modèle.txt")
